...
// ==========================================
// UPLOAD MEDIA (માત્ર સિમ્પલ અપલોડ, ફેસ ચેક વગર)
// ==========================================
...
// ==========================================
// FACE RECOGNITION SEARCH LOGIC (User Panel માટે)
// ==========================================
exports.searchByFace = async (req, res) => {
  try {
    ...
    // 1. Selfie ma thi face detect karo (TinyFaceDetectorOptions sathe)
    ...
    // 2. Database mathi badha photos laavo jema photo path hoy
    ...
    for (const item of galleries) {
      if (!item.face_descriptors || item.face_descriptors.length === 0) continue;
...
exports.updateMedia = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      const existingPaths = existingRecords.map(record =>
        isPhoto ? record.photo_path : record.video_path
      );
