...
import { Image as ImageIcon, Download, Share2, X, RefreshCw, Play } from 'lucide-react';
...
const GalleryPage = () => {
    ...
    // Helper to extract year safely from backend item
    const getItemYear = (item) => {
        ...
        if (item.event_date || item.created_at) {
            ...
            return isNaN(parsedYear) ? '' : String(parsedYear);
        }
        return '';
    };
    ...
    // =========================================================================
    // Dynamic Dependent Filters (Cascading Logic)
    // =========================================================================

    // 1. Available Years: If an event is selected, only show years that have that event
    const availableYears = useMemo(() => {
        let sourceList = mediaList;
        if (selectedEvent) {
            sourceList = mediaList.filter(
                (item) => getEventTitle(item) === selectedEvent
            );
        }
        const years = [
            ...new Set(sourceList.map((item) => getItemYear(item)).filter(Boolean)),
        ];
        return years.sort((a, b) => Number(b) - Number(a));
    }, [mediaList, selectedEvent]);

    // 2. Available Events: If a year is selected, only show events that occurred in that year
    const availableEvents = useMemo(() => {
        ...
        const events = [
            ...new Set(sourceList.map((item) => getEventTitle(item)).filter(Boolean)),
        ];
        return events.sort();
    ...
    const filteredMedia = useMemo(() => {
        return mediaList.filter((item) => {
            ...
            if (selectedYear) {
                matchYear = getItemYear(item) === selectedYear;
            }

            if (selectedEvent) {
                matchEvent = getEventTitle(item) === selectedEvent;
            }

            return matchYear && matchEvent;
        });
    }, [mediaList, selectedYear, selectedEvent]);
